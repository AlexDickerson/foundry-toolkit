...
//   POST /api/actors/:id/actions/:action → no-op stub that returns
//                                          `{ok: true}`; lets the SPA
//                                          exercise action buttons
//                                          without a backend
//   GET  /api/compendium/document        → synthetic document built from
//                                          the requested UUID's tail
//                                          segment, so formula detail
//                                          panels render populated state
  ...
  return {
    ...
    configureServer(server: ViteDevServer): void {
      ...
      server.middlewares.use((req, res, next) => {
        ...
        // → "Bomb Lesser"), so formula detail panels have *something* to
        // show in mock mode. Real resolution happens via the MCP bridge
        // in non-mock dev and in production.
        const compendiumDocMatch = /^\/api\/compendium\/document(?:\?.*)?$/.exec(url);
          ...
          sendJson(res, 200, {
            document: {
              ...
              system: {
                ...
                price: { value: { gp: 3 } },
                description: {
                  value: `<p>Mock description for <strong>${name}</strong>. Generated by the dev mock middleware so the SPA can render a populated detail panel without a live Foundry bridge.</p>`,
                },
              },
            },
          });
        ...
        // Generic action passthrough. The real bridge dispatches per
        // action; the mock just acks so the SPA's optimistic path works.
        const actionMatch = /^\/api\/actors\/([^/?]+)\/actions\/([^/?]+)(?:\?.*)?$/.exec(url);
