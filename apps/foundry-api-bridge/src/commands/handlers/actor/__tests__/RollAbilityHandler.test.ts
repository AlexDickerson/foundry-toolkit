...
describe('rollAbilityHandler', () => {
  ...
  describe('error handling', () => {
    it('should throw error if actor not found', async () => {
      ...
      await expect(rollAbilityHandler({ actorId: 'non-existent', ability: 'str' })).rejects.toThrow(
        'Actor not found: non-existent',
      );
    });

    it('should throw error for invalid ability key', async () => {
      await expect(rollAbilityHandler({ actorId: 'actor-123', ability: 'invalid' as AbilityKey })).rejects.toThrow(
        /Invalid ability key: invalid/,
      );
    });
    ...
    it('should throw error if roll returns empty array', async () => {
      ...
      await expect(rollAbilityHandler({ actorId: 'actor-123', ability: 'str' })).rejects.toThrow(
  ...
  describe('ability keys', () => {
    it('should accept all valid D&D 5e ability keys', async () => {
      for (const ability of ABILITY_KEYS) {
        ...
        await expect(rollAbilityHandler({ actorId: 'actor-123', ability })).resolves.toBeDefined();
      }
    });
